...
#input text file
readFlNm="C:\\Users\\anush\\Desktop\\college stuff\\NLP Assistantship\\input2.txt"
#precentage of summarization
percentageOfSummary=int(input("percentage of summary:"))
#fetch stop words
stopWords = set(stopwords.words('english'))
#dont update stop words
#stopWords.update(['"', "'", ':', '(', ')', '[', ']', '{', '}']) #'.',  ',', '?', '!', ';'
...
def obtainSummary(lineForCalc,lineForExtract,percentageOfSummary):
    ...
    for li in range(len(lineForExtract)):
        if(sentWtAndPriority[li][1]<numOfLinesInSummary) or re.search('[0-9]+',lineForExtract[li]):
    #            print(li,sentWtAndPriority[li])
            reducedSummary.append(lineForCalc[li])
    neww=""
    for res in reducedSummary:
        neww=neww+res
    return neww
...
def removeWeekNmMonthNm(NNP):
    ...
    for key in NNP.keys():
        if key.lower() in entries:
            print("Summary generated for the month of:",key)
            delKeys.append(key)
...
with open(readFlNm,"r",encoding="utf8") as fileIn:
        lineIn = fileIn.read()
...
print(len(linesOriginal2), 'lines org size')
print(len(linesReplacedPronn2), 'lines pronn2 size')
